...
MU_TEST(test_grid_create_and_init_1)
{
	...
	mu_assert_int_eq(TRANSPARENT, get_cell_color(gr, 0, 0));
...
MU_TEST(test_grid_get_cell_1)
{
	...
	mu_assert_int_eq(1, get_cell_color(gr, 0, 0));
	mu_assert_int_eq(2, get_cell_color(gr, 0, 1));
	mu_assert_int_eq(3, get_cell_color(gr, 0, 2));
	mu_assert_int_eq(4, get_cell_color(gr, 1, 0));
	mu_assert_int_eq(5, get_cell_color(gr, 1, 1));
	mu_assert_int_eq(6, get_cell_color(gr, 1, 2));
...
MU_TEST(test_grid_set_cell_1)
{
	...
	set_cell_color(gr, 0, 0, ORANGE);
	set_cell_color(gr, 1, 0, BLUE);

	mu_assert_int_eq(ORANGE, get_cell_color(gr, 0, 0));
	mu_assert_int_eq(BLUE, get_cell_color(gr, 1, 0));
...
MU_TEST(test_grid_scan_1)
{
	...
	game_score_t score = grid_scan(gr, (uint8_t *)&found);
...
MU_TEST(test_position_piece_in_grid_1)
{
	...
	mu_assert_int_eq(true, position_piece_in_grid(&pc, gr));
	mu_assert_int_eq(1, pc.col);
	mu_assert_int_eq(-2, pc.row);

	struct piece pc2 = { .row = 0,
			     .col = 1,
			     .colors = { PURPLE, RED, ORANGE } };
	piece_persist(&pc2, gr);

	mu_assert_int_eq(false, position_piece_in_grid(&pc, gr));
...
MU_TEST(test_piece_position_ok_1)
{
	...
	mu_assert_int_eq(ORANGE, get_cell_color(gr, 0, 0));
	mu_assert_int_eq(BLUE, get_cell_color(gr, 1, 0));
	mu_assert_int_eq(YELLOW, get_cell_color(gr, 2, 0));
...
MU_TEST(test_piece_move_fail_1)
{
	uint8_t _cells[] = {
		...
		TRANSPARENT, TRANSPARENT, TRANSPARENT, TRANSPARENT, TRANSPARENT,
		ORANGE,	     TRANSPARENT, TRANSPARENT, RED,	    TRANSPARENT,
		TRANSPARENT, BLUE,	  TRANSPARENT,
	};
	struct grid *gr = grid_alloc(6, 3);
	memcpy(gr->cells, &_cells, gr->size);
	struct piece pc = { .row = 3,
			    .col = 0,
...
MU_TEST_SUITE(test_suite_grid)
{
	...
	MU_RUN_TEST(test_position_piece_in_grid_1);
}
